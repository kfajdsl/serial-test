#include <packetize.h>

// CRC-8/CDMA2000
static const uint8_t crc_initial = 0xff;
static const uint8_t crc_table[256] = {
  
};

uint8_t crc8(uint8_t* bytes, size_t len) {
  uint8_t crc = crc_initial;

  while (len--) {
    crc = crc_table[*bytes++ ^ crc];
  }

  return crc;
} 

void cobs_encode(const uint8_t* data, size_t len, uint8_t* buf) {
  *buf++ = 0x0; // start delimted

  uint8_t* encoded = buf++; // cobs overhead
  *encoded = len + 1; // no zeroes yet
  uint8_t count = 0; // count till next zero

  while (len--) {
    count++;
    int val = *data++;
    if (val == 0x0) {
      *encoded = count;
      count = 0;
      *buf = len + 1;
    } else {
      *buf = val;
    }
    buf++;
  }
}

void cobs_decode(const uint8_t* data, size_t len, uint8_t* buf) {
  data++; // skip delimter
  uint8_t countdown = *data++; // countdown till next encoded byte & skip overhead
  len -= 2; // compensate for the above

  while (len--) {
    if (--countdown == 0) { // encoded zero
      *buf++ = 0x0;
      countdown = *data++;
    } else {
      *buf++ = *data++;
    }
  }
}

void packetize(const void* msg, size_t msg_len, uint8_t* buf) {
  uint8_t* msg_ptr = (uint8_t*) msg;
  size_t data_len = msg_len - 1;
  uint8_t* crc8_ptr = msg_ptr + data_len;
  *crc8_ptr = crc8(msg_ptr, data_len);

  cobs_encode(msg_ptr, msg_len, buf);
}

bool depacketize(const uint8_t* packet, size_t msg_len, void* msg) {
  cobs_decode(packet, msg_len + 2, (uint8_t*)msg); // msg_len doesn't include overhead and delimeter

  return crc8((uint8_t*)msg, msg_len) != 0;
}
